...
mkdir /tmp/$name
bash system/gather_system_info.sh | tee /tmp/$name/system_info.txt
bash docker/gather_docker_info.sh | tee /tmp/$name/docker_info.txt
bash agent/gather_agent_info.sh | tee /tmp/$name/agent_info.txt
bash agent/gather_fim_info.sh | tee /tmp/$name/fim_info.txt
...
cp /opt/threatstack/cloudsight/logs/threatstack-containers.log /tmp/$name/threatstack-containers.log
...
if [ $security_setting != "--send-unsafe"  ] ; then
    ...
    gpg --encrypt -r support@threatstack.com $name.tar.gz
fi
rm -rf /tmp/$name 
